...
static int camera_device_exist = 0;
...
static int scan_add_host(struct soc_camera_host *ici)
{
	...
	list_for_each_entry(icd, &devices, list) {
		if (icd->iface == ici->nr) {
			...
			if (ret) {
				if (camera_device_exist) {
					ret = 0;
				}
				mutex_unlock(&ici->host_lock);
				return ret;
			} else {
				camera_device_exist = 1;
			}
		}
	}
...
static int soc_camera_video_start(struct soc_camera_device *icd)
{
	...
	struct module_info *camera_module = (struct module_info *)(icd->link->priv);
	int ret;

	if (!icd->parent)
		return -ENODEV;

	ret = video_register_device(icd->vdev, VFL_TYPE_GRABBER, camera_module->video_devnum);
	if (ret < 0) {
