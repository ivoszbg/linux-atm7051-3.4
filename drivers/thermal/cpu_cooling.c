					...
					unsigned long event, void *data)
{
	...
	{
		//if hotplug out then hotplug in during COOLING, max_freq will be restore, so max_freq should be update to max in cooling status
		if (per_cpu(max_policy_freq, policy->cpu)) {
			max_freq = per_cpu(max_policy_freq, policy->cpu);
			if (policy->max != max_freq)
				cpufreq_verify_within_limits(policy, 0, max_freq);				
...
int leopard_cpufreq_is_ready,leopard_thermal_is_ready;
EXPORT_SYMBOL(leopard_cpufreq_is_ready);
extern int set_freq_clip_max(void);
int leopard_cpufreq_ready(void)
{
	if(leopard_thermal_is_ready == 1)
		return 0;
		
	if(leopard_cpufreq_is_ready == 0)
	{
		return -1;
	}
	else if(leopard_cpufreq_is_ready == 1)
	{
		//set freq_clip_max
		set_freq_clip_max();
		leopard_thermal_is_ready = 1;
		return 0;
	}
	return -1;
}
