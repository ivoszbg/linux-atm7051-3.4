...
static void __bio_split_by_idx(struct bio *bio, int idx,struct bio ** tail)
{
	...
	*tail = bio_clone(bio,GFP_NOIO|GFP_ATOMIC);
	...
	pr_debug(" __bio_split_by_idx  bio %p bio->bi_sector %lld, bio->bi_size %d b1->bi_idx %d bio->bi_vcnt %d bio->bi_phys_segments %d \n", b1,b1->bi_sector,b1->bi_size, b1->bi_idx,b1->bi_vcnt, b1->bi_phys_segments );
	b1 = *tail;		
	...
	pr_debug(" tail __bio_split_by_idx  bio %p bio->bi_sector %lld  bio->bi_size %d b1->bi_idx %d bio->bi_vcnt %d bio->bi_phys_segments %d \n", b1,b1->bi_sector,b1->bi_size, b1->bi_idx,b1->bi_vcnt, b1->bi_phys_segments );
}
extern void ext4_end_bio1(struct bio *bio, int error);
extern void ext4_end_bio(struct bio *bio, int error);
extern  void mpage_end_io(struct bio *bio, int err);
extern  void end_bio_bh_io_sync(struct bio *bio, int err);
extern void bounce_end_io_write(struct bio *bio, int err);
static void bio_split_by_idx(struct bio *bio, int idx, int count, struct bio ** page_bio, struct bio ** head, struct bio ** tail)
{
	struct bio * b1, * b2 ;
	...
	* page_bio = * head = * tail = NULL;
	pr_debug(" bio_split_by_idx  bio %p bio->bi_sector %lld,  bio->bi_size %d bio->bi_vcnt %d  \n", bio,bio->bi_sector,bio->bi_size, bio->bi_vcnt);
	pr_debug(" %pf %d\n",  bio->bi_end_io,count);
	if ( bio->bi_end_io == ext4_end_bio ) {
		dummy_end_io = ext4_end_bio1;
	}
	else if (bio->bi_end_io == mpage_end_io) {
		dummy_end_io = bi_end_io;
	}
	else if (bio->bi_end_io == end_bio_bh_io_sync) {
		dummy_end_io = bi_end_io;
	}
	else if (bio->bi_end_io == ext4_end_bio1) {
		dummy_end_io = bi_end_io;
	}
#ifdef CONFIG_BOUNCE	
	else if (bio->bi_end_io == bounce_end_io_write) {
		dummy_end_io = bi_end_io;
		BUG();
	}
#endif
	...
	if(  bio->bi_idx == idx ) {		//head
		...
		*tail = b1;			
		...
		if((*page_bio)->bi_sector + ((*page_bio)->bi_size>>9) != (*tail)->bi_sector) 
			BUG();
		goto out;
	}
	if ( idx + count ==  bio->bi_vcnt) { //tail		
		...
		* page_bio = b1;	
		...
		(*page_bio)->bi_end_io = dummy_end_io;
		(*page_bio)->bi_private = bi_private;	
		if((*head)->bi_sector + ((*head)->bi_size>>9) != (*page_bio)->bi_sector) 
			BUG();
		goto out;
	}
	__bio_split_by_idx(bio, idx,&b1);
...
static void extract_one_bio(struct bio * bio, unsigned int start_pfn, unsigned int end_pfn,
	...
	bio_for_each_segment(bv, bio,i) {			
		...
		{
			pr_debug("found  bio, %p  found_index  %d , count %d,  bio->bi_idx %d,(bio)->bi_vcnt %d \n", 
					bio, found_index, segment_cnt , bio->bi_idx ,bio->bi_vcnt);
			bio_split_by_idx(bio, found_index, segment_cnt, &page_bio, &head_bio, &tail_bio);	
			pr_debug ("found  bio %p page_bio %p , head_bio %p ,tail_bio %p\n", 
					bio, page_bio  , head_bio ,tail_bio);
	...
	if (found)  { //  the last bio
		pr_debug("found in tail  bio, %p  found_index  %d , count %d,  bio->bi_idx %d,(bio)->bi_vcnt %d \n", 
				bio, found_index, segment_cnt , bio->bi_idx ,bio->bi_vcnt);
		bio_split_by_idx(bio, found_index, segment_cnt, &page_bio, &head_bio, &tail_bio);	
		pr_debug("found  bio %p page_bio %p , head_bio %p ,tail_bio %p\n", 
			bio, page_bio  , head_bio ,tail_bio);
	...
	pr_debug("extract_one_bio bio %p synclist %d  async_list %d  \n", 
			bio, bio_list_size(synclist), bio_list_size(async_list) );
}
static void extract_bios(struct request * rq, unsigned int start_pfn, unsigned int end_pfn,
				struct bio_list * synclist, struct bio_list * async_list) {
...
extern void add_acct_request(struct request_queue *q, struct request *rq,
			     int where);
extern  struct request *get_request(struct request_queue *q, int rw_flags,
				   struct bio *bio, gfp_t gfp_mask);
static void submit_bio_list(struct request_queue *q, struct bio_list * blist, int flag) {
	struct bio * bio;
	struct bio_vec * bv;
	int i;
	unsigned int pfn;
...
static  void check_pending_async_bios(int start_pfn, int end_pfn,struct bio_list * sync_list, struct bio_list *async_list)
{
...
{
	struct bio * bio;
	struct requeue_work_struct  *requeue_work = container_of(__work, struct requeue_work_struct, work) ;
	struct bio_list  blist;
	bio_list_init(&blist);
	while(1) {
		spin_lock(&requeue_work->lock);		
...
static void requeue_bios(struct request_queue *q,struct bio_list * sync_list, struct bio_list *async_list) {
	struct bio * bio;
	int async_size = 0;
	struct requeue_work_struct  *requeue_work = &requeue_bio_work;
	pr_debug("requeue_bios  synclist %d  async_list %d  \n", bio_list_size(sync_list), bio_list_size(async_list) );
	submit_bio_list(q, sync_list,  REQ_FUA|REQ_SYNC|REQ_WRITE);
	...
	if (! async_size  && !bio_list_empty(&requeue_work->blist) ) {
		...
		printk(" async_list end requeue_work->blist  %d  \n", bio_list_size(&requeue_work->blist) );
	}	
}
static void cfq_remove_request1(struct request *rq)
{
	struct cfq_queue *cfqq = RQ_CFQQ(rq);
	if (cfqq->next_rq == rq)
		cfqq->next_rq = cfq_find_next_rq(cfqq->cfqd, cfqq, rq);
...
static int  dispatch_cfq_in_range(struct request_queue *q, struct cfq_queue * cfqq, unsigned int start_pfn, 
	unsigned int end_pfn,struct bio_list * sync_list, struct bio_list *async_list)
{
...
int boost_cma_requests(unsigned int start_pfn, unsigned int end_pfn)
{
	struct request_queue *q;
	...
	for (i = 0; i < IOPRIO_BE_NR; i++) {
		...
		if (cfqq) {			
			ret +=dispatch_cfq_in_range(q,cfqq, start_pfn,end_pfn,&sync_list, &async_list );	
		}
	}
	for (i = 0; i < IOPRIO_BE_NR; i++) {
...
static  void  scan_cma_range_work(struct work_struct  *__work) {
	int cnt;	
	struct delayed_work  *_work = container_of(__work, struct delayed_work, work) ;
	struct scan_work_struct  *scan_work = container_of(_work, struct scan_work_struct, work) ;
	cnt = boost_cma_requests(scan_work->start_pfn, scan_work->end_pfn);
	if ( cnt < 0) {
			return;
	}
	if( scan_work->done) {
...
void boost_cma_requests_start(unsigned int start_pfn, unsigned int end_pfn)
{
	struct scan_work_struct * scan_work = &scan_cma_work;
	struct requeue_work_struct  *requeue_work = &requeue_bio_work;
	
	if(!requeue_work->work.func) {
		printk(" requeue_bios  init work \n");
	...
	scan_work->done = 0;
	schedule_delayed_work((struct delayed_work *) scan_work, msecs_to_jiffies(0));
}
