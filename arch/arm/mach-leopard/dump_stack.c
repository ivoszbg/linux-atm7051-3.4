...
static int sstack_read_proc(char *page, char **start, off_t offset, int count, 
            int *eof, void *data)
{
...
static int sstack_write_proc(struct file *file, const char *buffer, 
...
static void sstack_create_proc(void)
{
    ...
    res = create_proc_entry("show_stack", S_IRUGO | S_IWUGO, NULL);
    if(res) {
