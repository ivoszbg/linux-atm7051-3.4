		...
		const int silent)
{
	...
	bh_backup = sb_bread(sb, nr_blocks - 1);
	if (bh_backup) {
		if (is_boot_sector_ntfs(sb, (NTFS_BOOT_SECTOR *)
...
static bool check_mft_mirror(ntfs_volume *vol)
{
	...
	DEBUG_ON(!mrecs_per_page);
	DEBUG_ON(!vol->mftmirr_size);
...
mft_unmap_out:
			...
			if (!ntfs_is_mft_recordp((le32 *)kmft)) {
				ntfs_error(sb, "$MFT error: Invalid mft "
						"record for %i.\n", i);
				goto mm_unmap_out;
			}
		}
		...
		if (((MFT_RECORD *)kmirr)->flags & MFT_RECORD_IN_USE) {
			...
			if (!ntfs_is_mft_recordp((le32 *)kmirr)) {
				ntfs_error(sb, "$MFT error: Invalid mft "
						"record for %i.\n", i);
				goto mm_unmap_out;
			}
		...
		if (bytes < sizeof(MFT_RECORD_OLD) ||
			...
			bytes = le32_to_cpu(((MFT_RECORD *)kmirr)->
					bytes_in_use);
			if (bytes < sizeof(MFT_RECORD_OLD) ||
					bytes > vol->mft_record_size ||
 ...
 * structure pointed to by @sfs (this is initialized with zeros before
 * ntfs_statfs is called). We interpret the values to be correct of the moment
 * in time at which we are called. Most values are variable otherwise and
 * this isn't just the free values but the totals as well. For example we
 * can increase the total number of file nodes if we run out and we
 * can keep doing this until there is no more space on the volume
 * left at all.
...
static int ntfs_statfs(struct dentry *dentry, struct kstatfs *sfs)
{
	...
	if(vol->free_clusters == -1)
		vol->free_clusters = get_nr_free_clusters(vol);
...
static const struct super_operations ntfs_sops = {
	...
	/*.clear_inode	= ntfs_clear_big_inode,*/	/* VFS: Called when an inode is
						   removed from memory. */
	.evict_inode	= ntfs_clear_big_inode,	/* VFS: Called when an inode is
						   removed from memory. */
...
static int ntfs_fill_super(struct super_block *sb, void *opt, const int silent)
{
	...
	*vol = (ntfs_volume) {
	...
	DEBUG_ON(blocksize != sb->s_blocksize);
...
struct kmem_cache *ntfs_ie_cache;
...
static const char ntfs_ie_cache_name[] = "ntfs_ie_cache";
...
static int __init init_ntfs_fs(void)
{
	...
	ntfs_ie_cache = kmem_cache_create(ntfs_ie_cache_name,
			(NTFS_MAX_NAME_LEN+1) * sizeof(ntfschar) +
			sizeof(INDEX_ENTRY), 0,
			SLAB_HWCACHE_ALIGN, NULL);
	if (!ntfs_name_cache) {
		printk(KERN_CRIT "NTFS: Failed to create %s!\n",
				ntfs_name_cache_name);
...
inode_err_out:
	kmem_cache_destroy(ntfs_ie_cache);
ie_err_out:
