...
#ifdef NTFS_RW
static int ntfs_create_file(struct inode *dir, struct dentry *dentry, int mode,
		       struct nameidata *nd)
{
	ntfschar *uname = NULL, *utarget = NULL;
	ntfs_inode *dir_ni = NTFS_I(dir), *ni;
	ntfs_volume *vol = dir_ni->vol;
	struct super_block *sb = vol->sb;
	mode_t type = (mode_t)(mode & ~07777) | S_IFREG;
	mode_t perm;
	int res = 0, uname_len;
...
static int ntfs_mkdir(struct inode *dir, struct dentry *dentry, int mode)
{
	...
	ntfs_volume *vol = dir_ni->vol;
	struct super_block *sb = vol->sb;
	mode_t type = (mode_t)(mode & ~07777) | S_IFDIR;
	mode_t perm;
	int res = 0, uname_len;
			 ...
			 u8 name_len, FILE_NAME_TYPE_FLAGS nametype)
{
	...
	if (!ni || !dir_ni || !name || !name_len ||
		...
		err = -EINVAL;
		ntfs_error(ni->vol->sb, "ntfs_link wrong arguments");
		goto err_out;
	}

	if ((ni->flag & FILE_ATTR_REPARSE_POINT)
	...
	fn->data_size = cpu_to_sle64(i_size_read(vi));
	read_unlock_irqrestore(&ni->size_lock, flags);
	
	fn->creation_time = utc2ntfs(vi->i_ctime);
	...
	err = ntfs_index_add_filename(dir_ni, fn, MK_MREF(ni->mft_no,
			ni->seq_no));
	if (err) {
		ntfs_error(ni->vol->sb, "Failed to add filename to the index");
		goto err_out;
	}
	/* Add FILE_NAME attribute to inode. */
	err = ntfs_attr_add(ni, AT_FILE_NAME, NULL, 0, (u8 *)fn, fn_len);
	if (err) {
		ntfs_error(ni->vol->sb, "Failed to add FILE_NAME attribute.\n");
	...
	return 0;
rollback_failed:
	ntfs_error(ni->vol->sb, "Rollback failed. Leaving inconsistent"
			" metadata.\n");
err_out:
	ntfs_free(fn);
	return err;
...
static int ntfs_link2(struct dentry *old_dentry, struct inode *dir,
	struct dentry *dentry)
{
	...
	dir_ni = NTFS_I(dir);
	res = ntfs_link_i(ni, dir_ni, uname, uname_len, FILE_NAME_POSIX);
	if (res)
		goto exit;

	vi->i_ctime = CURRENT_TIME_SEC;
	VFS_I(dir_ni)->i_mtime = CURRENT_TIME_SEC;
	VFS_I(dir_ni)->i_ctime = CURRENT_TIME_SEC;
	mark_inode_dirty(vi);
	mark_inode_dirty(VFS_I(dir_ni));
exit:
	/*
	...
	struct dentry *dentry)
{
	...
	res = ntfs_link_i(ni, dir_ni, uname, uname_len,
			FILE_NAME_POSIX);
	if (res)
		goto exit;
		       ...
		       struct inode *new_dir, struct dentry *new_dentry)
{
	...
	old_vi = old_dentry->d_inode;
	new_vi = new_dentry->d_inode;
	old_ni = NTFS_I(old_vi);
	if (new_vi) {
		ret = ntfs_unlink(new_dir, new_dentry);
		if (ret)
			goto out;
	}

	ret = ntfs_link2(old_dentry, new_dir, new_dentry);
	if (ret)
		goto out;

	ret = ntfs_unlink(old_dir, old_dentry);
	if (ret)
...
//	parent_dent = d_alloc_anon(parent_vi);
    parent_dent = d_obtain_alias(ntfs_iget(parent_vi->i_sb, parent_ino));
