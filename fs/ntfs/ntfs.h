...
#if 0
#define DEBUG_ON(cond)	do {		\
	if (cond) {					\
		printk(KERN_DEBUG "---- DEBUG_ON: %s:%u -----\n",	\
		       __func__, __LINE__);		\
		BUG();					\
	}						\
} while (0)
#else
/*just report it*/
#define DEBUG_ON(cond) WARN_ON(cond)
#endif
...
extern struct kmem_cache *ntfs_ie_cache;
...
extern const struct address_space_operations ntfs_nonresident_aops;
