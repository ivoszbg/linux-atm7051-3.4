...
MFT_RECORD *map_mft_record(ntfs_inode *ni)
{
	...
	if(!(bdev = ni->vol->sb->s_bdev) || !(disk = bdev->bd_disk) ||!(disk->flags & GENHD_FL_UP) )
		return ERR_PTR(-EIO);
	...
	if (atomic_read(&ni->map_count) >= 1) {
		atomic_inc(&ni->map_count);
...
MFT_RECORD *map_mft_record_lock(ntfs_inode *ni)
{
	MFT_RECORD *m;
	struct block_device *bdev;
	struct gendisk *disk;
...
void unmap_mft_record(ntfs_inode *ni)
{
	...
	ntfs_debug("Entering for mft_no 0x%lx.", ni->mft_no);
	atomic_dec(&ni->count);
	if (atomic_dec_return(&ni->map_count))
		return;
...
void unmap_mft_record_unlock(ntfs_inode *ni)
{
	struct page *page = ni->page;
		...
		MFT_RECORD *m, int sync)
{
	...
	do {
		...
		DEBUG_ON(!buffer_uptodate(bh));
		DEBUG_ON(!nr_bhs && (m_start != block_start));
		DEBUG_ON(nr_bhs >= max_bhs);
		bhs[nr_bhs++] = bh;
		DEBUG_ON((nr_bhs >= max_bhs) && (m_end != block_end));
...
static int ntfs_mft_bitmap_extend_allocation_nolock(ntfs_volume *vol)
{
	...
	DEBUG_ON(ll < rl2->vcn);
	DEBUG_ON(ll >= rl2->vcn + rl2->length);
...
int ntfs_inode_map_all_extents(ntfs_inode * ni)
{
	...
	for (i = 0; i < ni->nr_extents; i++) {
		...
		temp_m = map_mft_record(temp_ni);
		if(IS_ERR(temp_m)) {
			while (i > 0) {
				temp_ni = ni->ext.extent_ntfs_inos[i -1];
				unmap_mft_record(temp_ni);
				i--;
			}
...
void ntfs_inode_unmap_all_extents(ntfs_inode * ni)
{
	int i;
	ntfs_inode *temp_ni;
...
static int ntfs_mft_data_extend_allocation_nolock(ntfs_volume *vol)
{
	...
	ret = ntfs_inode_map_all_extents(mft_ni);
	if (ret) {
		unmap_mft_record(mft_ni);
		return ret;
	}
	...
	if (unlikely(IS_ERR(rl) || !rl->length || rl->lcn < 0)) {
		...
		if (!IS_ERR(rl))
			...
			ret = PTR_ERR(rl);
		ntfs_inode_unmap_all_extents(mft_ni);
		unmap_mft_record(mft_ni);
		return ret;
	}
	lcn = rl->lcn + rl->length;
...
have_alloc_rec:
	...
	ntfs_debug("Status of mft data after mft record initialization: "
			...
			(long long)i_size_read(vol->mft_ino),
			(long long)mft_ni->initialized_size);
	DEBUG_ON(i_size_read(vol->mft_ino) > mft_ni->allocated_size);
	DEBUG_ON(mft_ni->initialized_size > i_size_read(vol->mft_ino));
	read_unlock_irqrestore(&mft_ni->size_lock, flags);
mft_rec_already_initialized:
	...
	if (base_ni) {
		...
		atomic_inc(&ni->count);
		atomic_inc(&ni->map_count);
		atomic_inc(&vol->mft_ino->i_count);
		mutex_lock(&ni->mrec_lock);
		ni->page = page;
		ni->page_ofs = ofs;
		/*
...
int ntfs_mft_record_free(ntfs_inode *ni, MFT_RECORD *m)
{
	unsigned long mft_no = ni->mft_no;
	ntfs_volume *vol = ni->vol;
	int err;
	...
	if (ni->nr_extents > 0) {
		ntfs_error(vol->sb, "more extent mft record.");
		return -EINVAL;
	}
