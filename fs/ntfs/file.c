		...
		struct page **cached_page, struct pagevec *lru_pvec)
{
	...
	DEBUG_ON(new_init_size > ni->allocated_size);
	...
	DEBUG_ON(old_init_size != old_i_size);
	...
	DEBUG_ON(old_i_size != (loff_t)attr_len);
	/*
...
do_non_resident_extend:
	...
	read_lock_irqsave(&ni->size_lock, flags);
	DEBUG_ON(ni->initialized_size != new_init_size);
	read_unlock_irqrestore(&ni->size_lock, flags);
		...
		size_t *iov_ofs, size_t bytes)
{
	...
	do {
		...
		if (unlikely(copied != len)) {
			/* Do it the slow way. */
			addr = kmap(*pages);
			copied = __ntfs_copy_from_user_iovec_inatomic(
					addr + ofs, *iov, *iov_ofs, len);
			/*
			 * Zero the rest of the target like __copy_from_user().
			 */
		...
		s64 pos, size_t bytes)
{
	...
	DEBUG_ON(!a->non_resident);
	write_lock_irqsave(&ni->size_lock, flags);
	DEBUG_ON(end > ni->allocated_size);
		...
		const unsigned nr_pages, s64 pos, size_t bytes)
{
	...
	DEBUG_ON(attr_len != i_size);
	DEBUG_ON(pos > attr_len);
	end = pos + bytes;
	DEBUG_ON(end > le32_to_cpu(a->length) -
	...
	DEBUG_ON(end > ni->allocated_size);
	read_unlock_irqrestore(&ni->size_lock, flags);
	DEBUG_ON(initialized_size != i_size);
	if (end > initialized_size) {
		write_lock_irqsave(&ni->size_lock, flags);
		ni->initialized_size = end;
		i_size_write(vi, end);
