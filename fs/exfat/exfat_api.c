...
INT32 FsMountVol(struct super_block *sb)
{
	...
	if (drv >= MAX_DRIVE) {
		printk("EXFAT : Max number of mount points exceeded \n");
		sm_V(&z_sem);
		return(FFS_ERROR);
	}
...
/* FsGetparent : Get the parent directory */
INT32 FsGetparent(struct inode *inode, FILE_ID_T *fid)
{
	INT32 err;
	struct super_block *sb = inode->i_sb;
	FS_INFO_T *p_fs = &(EXFAT_SB(sb)->fs_info);

	/*  file id is not a valid pointer*/
	if (fid == NULL) return(FFS_INVALIDFID);

	/* acquire the lock for file system critical section */
	sm_P(&(fs_struct[p_fs->drv].v_sem));

	err = ffsGetParent(inode, fid);
