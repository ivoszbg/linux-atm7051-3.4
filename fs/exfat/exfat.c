...
/* ffsGetParent : Get the parent for the given child */
INT32 ffsGetParent(struct inode *inode, FILE_ID_T *fid)
{
	DENTRY_T *ep, *ep2;
	struct exfat_inode_info *exf_in = EXFAT_I(inode);
	struct super_block *sb = inode->i_sb;
	FS_INFO_T *p_fs = &(EXFAT_SB(sb)->fs_info);
	ENTRY_SET_CACHE_T *es=NULL;

	/*
	 * Get the minimum required information
	 * for the parent director
	 */
	fid->dir.dir = exf_in->fid.parent_dir.dir;
	fid->dir.size = exf_in->fid.parent_dir.size;
	fid->dir.flags = exf_in->fid.parent_dir.flags;
	fid->entry = exf_in->fid.parent_entry;
	fid->parent_entry = exf_in->fid.parent_entry;
...
INT32 ffsLookupFile(struct inode *inode, UINT8 *path, FILE_ID_T *fid)
{
	...
	/* "exfat_find_dir_entry()" returns FFS_DEVEJECT on surprise removal of device. If it returns -3 then return FFS_MEDIAERR*/
	if (dentry == FFS_DEVEJECT)
		return(FFS_MEDIAERR);
	if (dentry < -1)
		return FFS_NOTFOUND;
	...
	if (dentry == -1) {
		...
		fid->size = 0;
		fid->start_clu = p_fs->root_dir;
		fid->parent_dir.dir = p_fs->root_dir;;
		fid->parent_dir.size = 0;
		fid->parent_dir.flags = 0x01;
		fid->parent_entry = -1;
		...
		fid->parent_dir.dir = exf_in->fid.parent_dir.dir;
		fid->parent_dir.size = exf_in->fid.parent_dir.size;
		fid->parent_dir.flags = exf_in->fid.parent_dir.flags;
		fid->parent_entry = exf_in->fid.entry;
	...
	while (count > 0) {
		...
		if (clu == CLUSTER_32(~0)) {
			...
			if (last_clu == CLUSTER_32(~0)) {
				...
				if ((new_clu.flags == 0x01) && (FAT_write(sb, last_clu, new_clu.dir) == FFS_MEDIAERR))
					return FFS_MEDIAERR;
			}
...
INT32 ffsTruncateFile(struct inode *inode, UINT64 new_size)
{
	...
	ENTRY_SET_CACHE_T *es=NULL;
	UINT64 aligned_old_file_size=0;
	UINT64 aligned_new_file_size=0;

	if(fid->type == TYPE_DELETED)
	...
	aligned_old_file_size = inode->i_blocks<<9;
	aligned_new_file_size = ((i_size_read(inode) + p_fs->cluster_size - 1))&~((loff_t)p_fs->cluster_size-1);
	if(aligned_old_file_size<=aligned_new_file_size)
		return(FFS_SUCCESS);

	fs_set_vol_flags(sb, VOL_DIRTY);

	clu.dir = fid->start_clu;
	clu.size = (INT32)((aligned_old_file_size) >> p_fs->cluster_size_bits);
	clu.flags = fid->flags;

	if (new_size > 0) {
		num_clusters = (INT32)((aligned_new_file_size) >> p_fs->cluster_size_bits);
	...
	if (last_clu != CLUSTER_32(0)) {
		if ((fid->flags == 0x01) && (FAT_write(sb, last_clu, CLUSTER_32(~0)) == FFS_MEDIAERR))
			return FFS_MEDIAERR;
	}
...
INT32 ffsRemoveFile(struct inode *inode, FILE_ID_T *fid)
{
	...
	/* Added to omit ffsTruncateFile call from exfat_evict_inode */
	fid->type = TYPE_DELETED;
...
INT32 ffsGetStat(struct inode *inode, DIR_ENTRY_T *info)
{
	...
	*(uni_name.name) = 0x0;
	...
	p_fs->fs_func->get_uni_name_from_ext_entry(sb, &(fid->dir), fid->entry, uni_name.name, &faulty_entry);
...
INT32 ffsReadDir(struct inode *inode, DIR_ENTRY_T *dir_entry)
{
	...
	while (clu.dir != CLUSTER_32(~0)) {
		...
		for ( ; i < dentries_per_clu; i++, dentry++) {
			...
			*(uni_name.name) = 0x0;
			faulty_entry = 0;
			p_fs->fs_func->get_uni_name_from_ext_entry(sb, &dir, dentry, uni_name.name, &faulty_entry);
			if(faulty_entry == 0x1) {
				printk("Filesystem is corrupted ...Please run fsck\n");
			...
			if (!ep)
				return FFS_MEDIAERR;
			nls_cstring_to_uniname(sb, &uni_name , dir_entry->Name, &lossy);
			strm_ep = (STRM_DENTRY_T *)ep;
			strm_name_hash = GET16_A(strm_ep->name_hash);
			strm_name_len =  strm_ep->name_len;
			if ((uni_name.name_hash != strm_name_hash) || 
					(uni_name.name_len != strm_name_len)) {
				printk("Filesystem is corrupted ...Please run fsck\n");
				continue;
			}
...
INT32 clear_cluster(struct super_block *sb, UINT32 clu)
{
	...
	for ( ; s < n; s++) {
		...
		brelse(tmp_bh);
		tmp_bh = __getblk(sb->s_bdev, s, 1 << p_bd->sector_size_bits);
		if (!tmp_bh)
			return(FFS_MEDIAERR);
		memset (tmp_bh->b_data, 0x0, 1 << p_bd->sector_size_bits);
		lock_buffer(tmp_bh);
		set_buffer_uptodate(tmp_bh);
		mark_buffer_dirty(tmp_bh);
		unlock_buffer(tmp_bh);
		write_dirty_buffer(tmp_bh, WRITE);
		buffer_uptodate (tmp_bh);		
	}
...
INT32 exfat_alloc_cluster(struct super_block *sb, INT32 num_alloc, CHAIN_T *p_chain)
{
	...
	while ((new_clu = test_alloc_bitmap(sb, hint_clu-2)) != CLUSTER_32(~0)) {
		...
		if ((p_chain->flags == 0x01) && (FAT_write(sb, new_clu, CLUSTER_32(~0)) == FFS_MEDIAERR))
			goto out;

		if (p_chain->dir == CLUSTER_32(~0)) {
			p_chain->dir = new_clu;
		} else {
			if ((p_chain->flags == 0x01) && (FAT_write(sb, last_clu, new_clu) == FFS_MEDIAERR));
				goto out;
		}
		last_clu = new_clu;
	...
	return(num_clusters);
out:
	return FFS_DEVEJECT; /* Return -ve number as any positive number can be success case */
...
void exfat_chain_cont_cluster(struct super_block *sb, UINT32 chain, INT32 len)
{
	...
	while (len > 1) {
		if(FAT_write(sb, chain, chain+1) == FFS_MEDIAERR)
			return;
		chain++;
		len--;
	}
	if(FAT_write(sb, chain, CLUSTER_32(~0)) == FFS_MEDIAERR)
		return;
...
INT32 find_empty_entry(struct inode *inode, CHAIN_T *p_dir, INT32 num_entries)
{
	...
	while ((dentry = search_deleted_or_unused_entry(sb, p_dir, num_entries)) < 0) {
		if (p_fs->dev_ejected)
			return (FFS_DEVEJECT);  /* Added to make common return from this function on surprise removal of block device */
		...
		if ((clu.flags == 0x01) && (FAT_write(sb, last_clu, clu.dir) == FFS_MEDIAERR))
			return FFS_DEVEJECT;
...
void exfat_get_uni_name_from_ext_entry(struct super_block *sb, CHAIN_T *p_dir, INT32 entry, UINT16 *uniname, INT32 *faulty_entry)
{
	INT32 i;
    INT32 num_ext_entries;
	INT32 order = 1;
	DENTRY_T *ep;
	ENTRY_SET_CACHE_T *es;
	FILE_DENTRY_T *file_ep;
	FS_INFO_T *p_fs = &(EXFAT_SB(sb)->fs_info);
    file_ep = (FILE_DENTRY_T *) get_entry_in_dir(sb, p_dir, entry, NULL);
	if (!file_ep)
		return;
	num_ext_entries = file_ep->num_ext;
