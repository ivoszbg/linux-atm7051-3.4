...
/* sync mode：
0：asynchronization sync(queue_work)
1：synchronization sync(function call) */
static int sync_mode=0;
static int sync_mode_flag=0;
...
int get_sync_mode(void)
{
	...
	if(sync_mode_flag == 0)
	{
		ret = get_config("sync.mode", (char *)(&mode), sizeof(unsigned int));
		if (ret == 0) {
			printk("sync.mode: %d from config.xml\n", mode);
			sync_mode = mode;
		}
		else
		{
			printk("no sync.mode in config.xml\n");
		}
		sync_mode_flag = 1;
	}
	return sync_mode;
